...
from omnizart.music.labels import (
    LabelType, MaestroLabelExtraction, MapsLabelExtraction, MusicNetLabelExtraction, PopLabelExtraction, GukakLabelExtraction
)
...
class MusicTranscription(BaseTranscription):
    ...
    needed got override.
    """
    def __init__(self, conf_path=None, model_path=None):
        ...
        self.model, self.model_settings = self._load_model(model_path, custom_objects=self.custom_objects)

    def transcribe(self, input_audio, output="./"):
        ...
        # print('down_fs: ', model_settings.feature.sampling_rate)
        # print('hop: ', model_settings.feature.hop_size)
        # print('win_size: ', model_settings.feature.window_size)
        # print('fr: ', model_settings.feature.frequency_resolution)
        # print('fc: ', model_settings.feature.frequency_center)
        # print('tc: ', model_settings.feature.time_center)
        # print('g: ', model_settings.feature.gamma)
        # print('bin_per_octave: ', model_settings.feature.bins_per_octave)
        # print('harmonic_num: ', model_settings.feature.harmonic_number)
        # print('harmonic: ', model_settings.feature.harmonic)
        ...
        # print(model.summary())
        pred = predict(feature[:, :, channels], model) # 이 라인에서 list index out of range 가 나온다.
        # print(pred.shape)
        ...
        midi = multi_inst_note_inference(
            ...
            channel_program_mapping=[0],
        )
        
#         print('pred', pred.shape)
#         print(midi.instruments)
        
#         print('mode: ', model_settings.training.label_type)
#         print('onset_th: ', model_settings.inference.onset_th)
#         print('dura_th: ', model_settings.inference.dura_th)
#         print('frame_th: ', model_settings.inference.frame_th)
#         print('inst_th: ', model_settings.inference.inst_th)
#         print('channel_program_mapping: ', self.mode_inst_mapping[model_settings.transcription_mode])
    ...
    def generate_feature(self, dataset_path, music_settings=None, num_threads=4):
        ...
        settings.training.label_type = 'note'
        ...
        if dataset_type is None:
            logger.warning(
                "the new warning here The given path %s does not match any built-in processable dataset. Do nothing...",
                dataset_path
            )
            struct = d_struct.GukakStructure
            label_extractor = GukakLabelExtraction
        ...
        # Feature extraction
        logger.info(
            "Start extract the feature of the dataset."
            "This may take time to finish and affect the computer's performance.",
        ) # 여기까지가 시작하면 여기 까지 실행 먼저됨. 그러고 progress: 19/1614가 실행됨.
        ...
        # Extract labels
        logger.info("Start extracting the label of the dataset")
    ...
    def train(self, feature_folder, model_name=None, input_model_path=None, music_settings=None):
        ...
        """
        # print('music_settings: ', music_settings)
        settings = self._validate_and_get_settings(music_settings)
        ...
        with strategy.scope():
            ...
            # print('settings.training.label_type: ', settings.training.label_type)
            ...
            # print('out_class: ', l_type.get_out_classes())
            # print('ch_num: ', len(settings.training.channels))
        ...
        callbacks = [
            tf.keras.callbacks.EarlyStopping(patience=settings.training.early_stop, monitor='val_accuracy'),
...
def _parallel_feature_extraction(audio_list, out_path, feat_settings, num_threads=4): # _parallel_feature_extraction(train_wav_files, train_feat_out_path, settings.feature, num_threads=num_threads)
    feat_extract_params = {
    ...
    # print('extracting....')
    # print('여기인듯', feat_settings.bins_per_octave)
    iters = enumerate(
        parallel_generator(
            extract_cfp_feature,
            audio_list,
...
class MusicDatasetLoader(BaseDatasetLoader):
    ...
    def __init__(
        ...
        print(list(self.hdf_refs.values()))
